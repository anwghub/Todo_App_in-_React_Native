...
const index = () => {
  ...
  const addTodo = async () => {
    try {
      ...
      axios
        .post("http://192.168.1.100:3000/todos/665243e5e049f8a9c822bb6e", todoData)
        .then((response) => {
  ...
  const getUserTodos = async () => {
    try {
      const response = await axios.get(
        `http://192.168.1.100:3000/users/665243e5e049f8a9c822bb6e/todos`
      );
  ...
  const markTodoAsCompleted = async (todoId: any) => {
    try {
      const response = await axios.patch(
        `http://192.168.1.100:3000/todos/${todoId}/complete`
      );
      console.log(response.data);
      
      // Update completedTodos and pendingTodos after marking todo as completed
      const updatedPendingTodos = pendingTodos.filter(todo => todo._id !== todoId);
      const completedTodo = pendingTodos.find(todo => todo._id === todoId);
        ...
        <View style={{ padding: 10 }}>
          {todos?.length > 0 ? (
            <View>
              ...
              {pendingTodos?.map((item:any, index) => (
                <Pressable
                  onPress={() => {
                    router?.push({
                      pathname: "/home/info",
              ...
              {completedTodos?.length > 0 && (
                <View>
                  ...
                  {completedTodos?.map((item: any, index) => (
                    <Pressable
                      style={{
                        backgroundColor: "#E0E0E0",
                        padding: 10,
